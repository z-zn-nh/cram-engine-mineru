...
| `/ingest` | 扫描当前文件夹资料，索引 md/txt，并用 MinerU 解析 PDF/图片/PPT；PPT 失败时可用 LibreOffice 转 PDF 兜底 |
...
| `/config` | 重新打开 LLM 配置页，更换 API key、Base URL 或模型 |
...
这个配置是全局复用的，不跟随学科文件夹。之后在其他课程文件夹里运行 `cram` 会继续使用同一套 key、Base URL 和模型。需要更换时，在 TUI 输入：

```text
/config
```

Base URL 必须是纯 URL，例如 `https://api.openai.com/v1`，不要粘贴成 Markdown 链接格式。
...
主解析引擎仍然是 MinerU。当前 `/ingest` 会：

- 直接把当前文件夹里的 `md/txt` 笔记切块写入 `.cram/index/chunks.jsonl`
- 对 `pdf/png/jpg/jpeg` 调用 `mineru -p <file> -o .cram/parsed/<file>`，再把 MinerU 生成的 Markdown 作为原始资料引用索引
- 复用 `.cram/parsed/` 中未过期的 MinerU Markdown，避免大 PDF 每次重复 OCR
- 对 `ppt/pptx` 先尝试 MinerU 原生解析；如果失败且本机有 LibreOffice/`soffice`，会转成 PDF 后再交给 MinerU 解析

问答时会检索 `.cram/index/chunks.jsonl`，把命中的片段加入 LLM 上下文，并要求模型用类似 `[老师重点.pdf:mineru:1]` 的标签引用来源。
...
PPT/PPTX 兜底转换需要安装 LibreOffice，并确保 `soffice` 在 PATH 中；如果不在 PATH，可以设置 `CRAM_LIBREOFFICE_BIN` 指向 `soffice.exe`。
